...
- **Images need a one-time ~15 MB `eng.traineddata` download**; nothing is bundled. The traineddata lives in liteparse's own cache dir, *not* in the wheel — an earlier reading of this repo claimed it was embedded in the `.so`, which was wrong (the `unicharset`/`lstm-punc-dawg` strings in the binary are Tesseract's symbol names). OCR is also English-only unless `ocr_language` is passed, so a Cyrillic or CJK scan currently returns plausible-looking Latin garbage rather than an error
